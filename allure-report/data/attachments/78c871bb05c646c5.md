...
- Name: Morevalidation.spec.js >> Pop up validations
- Location: tests\Morevalidation.spec.js:5:1
        ...
        - banner [ref=f1e3]:
          - generic [ref=f1e6]:
            - generic [ref=f1e7]:
              ...
              - generic [ref=f1e10]: 2d 12h 41m 23s
...
```ts
  ...
  5  | test('Pop up validations', async ({page})=>
  6  | {
...
> 14 |     await page.locator("#mousehover").hover();
     |                                       ^ Error: locator.hover: Target page, context or browser has been closed
  15 |     const framepage = page.frameLocator("#courses-iframe");
  16 |     await framepage.locator("li a[href*='lifetime-access']:visible").click();
  17 |    const textcheck=  await framepage.locator(".text h2").textContent();
  18 |    const subs= textcheck.split(" ")[1];
  19 |    console.log(subs);
  20 | 
  21 | });
  22 | test('screenshots and visual comparision', async ({page})=>
  23 | {
  ...
  26 |     await page.locator("#hide-textbox").click();
  27 |     await page.locator("#hide-textbox").screenshot({path: 'partialscreenshot.png'});
  28 |     await page.screenshot({path: 'screenshot.png'});
  29 |     await expect(page.locator("#displayed-text")).toBeHidden();
  30 | });
  31 | test('visual comparision', async ({page})=>
  32 | {
  33 |     await page.goto("https://rahulshettyacademy.com/AutomationPractice/");
  34 |     expect(await page.screenshot()).toMatchSnapshot('landing.png')
  35 | });
```